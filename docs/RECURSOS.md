...
**Resumen**: de las 15 tablas que toca la aplicación, 6 son nuevas del módulo de inducción y las crea por completo `SistemaInduccion_SetupCompleto.sql`; las otras 9 pertenecen al esquema de captación que ya existía en la base de datos — el script no las crea (no le corresponde), pero sí siembra datos de prueba dentro de ellas donde hace falta (`Usuarios`, `Periodos` si no hay ninguno, y las filas de catálogo de `TiposDocumentos`/`EstadosDocumentos` que se crean solas desde la aplicación). Para que el proyecto funcione de cero, la única condición externa real es que la base ya tenga su esquema de captación con al menos los 4 `Roles` cargados (`Databasenew.sql` es la referencia de ese esquema).
